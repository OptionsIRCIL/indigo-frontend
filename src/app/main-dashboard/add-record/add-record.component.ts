	...
	template: `<mat-form-field class="chip-list" appearance="fill">
		...
		<mat-chip-grid #chipGrid aria-label="Alias List">
			@for (alias of aliases; track alias) {
				<mat-chip-row [removable]="removable" (removed)="remove(alias)">
					{{ alias.name }}
					@if (removable) {
...
export let additionalDisabilityList = [
	"Disability A",
	"Disability B",
	"Disability C",
];
...
export class IndividualContentDialog {
    ...
    private recordIdState: RecordIdState,
		...
		if (data?.mode == "e") {
			this.titleText = "Edit Record - Individual";
			this.mode = "e";
		} else {
			this.titleText = "Add Record - Individual";
		}
	}
  ...
  id!: string;
    ...
    this.showDisabilityList = this.form.get("hasDisability")?.value ?? "";
		...
		this.form.get("withheldAddress")?.valueChanges.subscribe((disabled) => {
			const fields = [
				this.form.get("addressInfo")!,
				this.form.get("addressInfo2")!,
				this.form.get("city")!,
				this.form.get("state")!,
			];
			fields.forEach((f) => {
				disabled ? f.disable() : f.enable();
			});
		});
...
export class OrganizationContentDialog {
	...
	constructor(
		...
		if (data?.mode == "e") {
			this.titleText = "Edit Record - Organization";
			this.mode = "e";
		} else {
			this.titleText = "Add Record - Organization";
		}
	}
