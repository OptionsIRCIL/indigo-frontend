...
export class ConsumerInformationFileComponent {
	...
	private saveStoredEfforts(formId: string, rows: EmployeeEffort[]): void {
		...
		localStorage.setItem(
			`${this.effortStoragePrefix}${formId}`,
	...
	onSave(): void {
		if (this.config.demoMode) {
			...
			const payload: CifStorageForm = {
				...
				activationDate: this.toDateValue(
					this.consumerForm.value.activationDate,
				),
				...
				independentLivingPlanActivity:
					this.consumerForm.value.independentLivingPlanActivity ?? "",
				dateofILPAction: this.toDateValue(
					this.consumerForm.value.ilpActionDate,
				),
				individual:
					this.recordType === "individual"
						? (this.recordId ?? "")
