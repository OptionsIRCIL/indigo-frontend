...
export class CEOComponent {
	...
	get filteredCategories(): string[] {
		...
		return this.categories.filter(
			(c) =>
				c.toLowerCase().includes(value) && !this.selectedCategories.includes(c),
		);
	}
	...
	removeFutureReference(reference: string) {
		this.selectedFutureReferences = this.selectedFutureReferences.filter(
			(r) => r !== reference,
		);
	}
		...
		if (this.config.demoMode) {
			...
			const payload: CeoStorageForm = {
				...
				personsWithDisabilities:
					this.ceoForm.value.numPersonsWithDisabilities ?? 0,
